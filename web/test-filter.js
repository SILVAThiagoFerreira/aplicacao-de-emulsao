...
import fs from 'fs';
import { applyFilters, buildDailyTable, buildPlanCombinedWaterfallRows, buildPlanDailyTable, buildPlanWaterfallRows, totals } from './src/lib/aggregate.js';
...
const combinedWaterfall = buildPlanCombinedWaterfallRows(planRows);
...
assert.equal(combinedWaterfall[0].value, planRows[0].emulsao, 'Expected the combined waterfall height to use emulsion');
assert.equal(combinedWaterfall[0].furos, planRows[0].furos, 'Expected the combined waterfall to carry daily holes');
assert.equal(combinedWaterfall.at(-1).value, planTotal.emulsao, 'Expected the combined waterfall total height to use emulsion');
assert.equal(combinedWaterfall.at(-1).furos, planTotal.furos, 'Expected the combined waterfall total to carry total holes');
...
const syntheticCombinedWaterfall = buildPlanCombinedWaterfallRows(syntheticRows);
...
assert.equal(syntheticCombinedWaterfall[1].furos, 4, 'Expected the combined waterfall to align holes with the second day');
assert.equal(syntheticCombinedWaterfall.at(-1).furos, 7, 'Expected the combined waterfall total to be 7 holes');
