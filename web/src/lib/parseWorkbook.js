...
export function parseWorkbookToDashboard(buffer, { sourceUrl = '', finalUrl = '' } = {}) {
...
function excelSerialToDate(serial) {
  ...
  return !Number.isNaN(date.getTime()) ? date.toISOString().slice(0, 10) : String(serial);
}

function normalizeMonth(value) {
  if (typeof value === 'number' && Number.isFinite(value)) return Math.min(Math.max(Math.round(value), 1), 12);
